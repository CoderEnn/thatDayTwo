        ...
        if segue.identifier == "newEventSegue" {
            ...
            if segue.destinationViewController.isKindOfClass(NewEventViewController.self) {
                ...
                vc.clouser = {person in
                    let personArr = AVObject(className: "Idd")
//                   let personArr = AVObject(className: "Pobek")
                    personArr.setObject(person.event, forKey: "event")
                    
                    personArr.setObject(person.lasting, forKey: "lasting")
                    
                    personArr.setObject(person.remind, forKey: "remind")
                    

                    personArr.saveInBackgroundWithBlock({ (_, error) in
                        if error != nil {
                        print(error)
                        }
                    })
